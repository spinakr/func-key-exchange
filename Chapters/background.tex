...
Generates a private key $k_id$ from a given $id \in \{0,1\}^*$, using $msk = s$, as following. Calculate $h_id = H_1(id) \in G_1^*$, and set private key $k_id = h_id^s$. This is now the private key of the user with the corresponding id.

\end{description}
\caption{\Gls{ibe}, Boneh and Franklin \cite{DBLP:ibe}}
\label{fig:ibenc2}
\end{mdframed}
\end{figure}
